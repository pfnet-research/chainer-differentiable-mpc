...
from util import xpbmv, xpbger, xpbatch_lu_factor, xpbatch_lu_solve, get_array_module
import copy
...
class LQR_active:
    ...
    def __init__(self, x_init, C, c, large_f, f, T, n_state, n_ctrl, u_zero_Index=None):
        ...
        self.xp = get_array_module(x_init)
    ...
    def backward(self):
        ...
        for t in range(self.T - 1, -1, -1):
            ...
            if True:
                ...
                index_qt_ux = self.xp.repeat(self.xp.expand_dims(index, axis=2), Qt_ux.shape[2], axis=2)
            ...
            Ks.append(Kt)
            ks.append(kt)
            Vt = Qt_xx + self.xp.matmul(Qt_xu, Kt) + self.xp.matmul(Kt_T, Qt_ux) + self.xp.matmul(
                self.xp.matmul(Kt_T, Qt_uu), Kt)
            vt = qt_x + xpbmv(Qt_xu, kt) + xpbmv(Kt_T, qt_u) + xpbmv(self.xp.matmul(Kt_T, Qt_uu), kt)
        ...
        for t in range(self.T):
            ...
            if self.u_zero_Index is not None:
                assert self.u_zero_Index[t].shape == new_ut.shape, str(self.u_zero_Index[t].shape) + " : " + str(
                    new_ut.shape)
                new_ut = self.xp.where(self.u_zero_Index[t], self.xp.zeros_like(new_ut), new_ut)
                assert list(new_ut.shape) == [self.n_batch, self.n_ctrl], "actual:" + str(new_ut.shape)
            new_u.append(new_ut)
            if t < self.T - 1:
